...
def create_file(total_line_count, lock, file_name, line_cnt=10, column_cnt=3):
    file_to_create = open(file_name, "w")
    for line_number in range(line_cnt):
        if line_number != line_cnt - 1:
            file_to_create.write(get_randomn_line_entries(column_cnt) + NEW_LINE_CHAR)
        else:
            file_to_create.write(get_randomn_line_entries(column_cnt))
        with lock:
    ...
    with multiprocessing.Manager() as manager:
        ...
        for file_count_id in range(file_count):
            ...
            t = multiprocessing.Process(
                target=create_file,
                args=(total_line_count, lock, file_name_with_suffix, line_count, column_count))
            jobs.append(t)
        ...
        print "Process Completed; total_line_count=", total_line_count.value
